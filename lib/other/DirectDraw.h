...
namespace Direct
{
	...
	public:
		...
		void BltFrom (Surface & src, RECT * dstRect = 0, RECT * srcRect = 0, DWORD dwFlags = 0)
		{
			assert (_i != 0);
			HRESULT res = _i->Blt (dstRect, src._i, srcRect, dwFlags, 0);
			if(res != DD_OK) 
	...
	class CBitmap: public Surface
	{
	public:
		CBitmap () { };
		CBitmap (Draw& draw, int id) { Load(draw, id); }

		void Load(Draw& draw, int id)
		{
			::Bitmap bmp;

			int width, height;
			bmp.Load(GetModuleHandle(NULL), id);
		
			bmp.GetSize(width, height);
			OffScreenSurface srf(draw, width, height);

			HDC hDC = srf.GetDC();
			bmp.BlitTo(hDC, width, height);
			srf.ReleaseDC(hDC);

			bmp.Release();

			IFace<IDirectDrawSurface>::operator = (srf);
		}
	};
